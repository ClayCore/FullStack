...
const updateArticleValidations = [
    ...
    check('title', 'toast.article.title_too_long').isLength({
        max: ARTICLE_TITLE_MAX_LENGTH,
    }),
    check('content', 'toast.article.content_too_short').isLength({
        min: ARTICLE_CONTENT_MIN_LENGTH,
    }),
    check('content', 'toast.article.content_too_long').isLength({
        max: ARTICLE_CONTENT_MAX_LENGTH,
    }),
];
