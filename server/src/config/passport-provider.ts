...
passport.deserializeUser(
    (id: any, done: (err: Error, user: UserDocument) => void) => {
...
passport.use(
    new LocalStrategy(
        ...
        (email, password, done): void => {
            console.log(
                `Using [LocalStrategy]. Using [email]:[password]\n\t[${email}]:[${password}]`
            );
            ...
            UserCollection.findOne(
                ...
                (err: Error, user: UserDocument): void => {
                    ...
                    if (!user) {
                        return done(
                            { message: 'toast.user.email_not_found' },
                            false
                        );
                    }
                    ...
                    user.comparePassword(
                        ...
                        (err: Error, isMatch: boolean) => {
                            ...
                            return done(
                                { message: 'toast.user.password_error' },
                                false
                            );
                        }
                    );
...
function verifyClient(
    clientId: string,
    clientSecret: string,
    done: (error: Error | undefined, user?: any) => void
) {
    console.log(
        `Using [ClientPasswordStrategy]. Using [clientId]:[clientSecret]\n\t[${clientId}]:[${clientSecret}]`
    );

    const client: Client | undefined = ClientCollection.find(
        (value: Client) => value.id === clientId
    );
...
passport.use(
    new BearerStrategy(
        (
            ...
            console.log(
                `Using [BearerStrategy]. Using [accessToken]\n\t[${accessToken}]`
            );

            AccessTokenCollection.findOne(
                { token: accessToken },
                (error: Error, token: AccessToken): void => {
                    if (error) {
                        return done(error);
                    }
                    ...
                    if (token.userId) {
                        ...
                        const client:
                            | Client
                            | undefined = ClientCollection.find(
