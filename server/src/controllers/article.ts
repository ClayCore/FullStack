import { DEFAULT_PAGE_SIZE } from '@flux/shared/constants';
import { RequestHandler, Request, Response, NextFunction } from 'express';
import { validationErrorResponse } from './utils';
import { validationResult } from 'express-validator';
import Article from '@flux/shared/models/Article';
import ArticleCollection from '@flux/shared/models/Article/ArticleCollection';
import ArticleDocument from '@flux/shared/models/Article/ArticleDocument';
import ArticleResponse from '@flux/shared/models/Response/ArticleResponse';
import CommentCollection from '@flux/shared/models/Comment/CommentCollection';
import InteractionType from '@flux/shared/models/InteractionType';
import NotificationCollection from '@flux/shared/models/Notification/NotificationCollection';
import NotificationDocument from '@flux/shared/models/Notification/NotificationDocument';
...
export const remove: RequestHandler = (
    ...
    ArticleCollection.findById(req.params.id)
        .exec()
        .then((article: ArticleDocument | null) => {
            if (!article) {
                return Promise.reject(
                    res.status(404).json({ message: 'toast.article.not_found' })
                );
            }
...
export const update: RequestHandler = (
    ...
    ArticleCollection.findById(req.body._id)
        .exec()
        .then((article: ArticleDocument | null) => {
            if (!article) {
                return Promise.reject(
            ...
            return ArticleCollection.findByIdAndUpdate(req.body._id, {
...
export const like: RequestHandler = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    ...
    ArticleCollection.findById(req.query.id)
        .exec()
        .then((article: ArticleDocument | null) => {
            if (!article) {
                return Promise.reject(
            ...
            if (Number.parseInt(req.query.rating as string) === 1) {
                ...
                const toRemove: number = likes.findIndex(
                    (value: string) => value === user._id.toString()
                );
                likes.splice(toRemove, 1);
            } else {
                return Promise.reject(res.status(400).end());
            }
            return ArticleCollection.findByIdAndUpdate(req.query.id, {
                likes: likes,
            }).exec();
        })
        .then((updated: Article | null) => {
            ...
            const notification: NotificationDocument = new NotificationCollection(
                {
                    ...
                    event:
                        Number.parseInt(req.query.rating as string) === 1
...
export const create: RequestHandler = (
    ...
    article
        ...
        .then((saved: Article) => {
            ...
            if (
                req.body.mentions &&
                (req.body.mentions as string[]).length > 0
            ) {
...
export const read: RequestHandler = async (
    ...
    const latestTime: Date = req.query.latest
        ? new Date(req.query.latest as string)
        : new Date(Date.now());
    const pageSize: number = req.query.size
        ? Number.parseInt(req.query.size as string)
        : DEFAULT_PAGE_SIZE;

    const findAuthorInUsers = (
        article: Article
    ): Promise<UserDocument | null> => {
        return UserCollection.findById(article.author).exec();
    };

    const articles: ArticleDocument[] = await ArticleCollection.find({
        createdAt: { $lt: latestTime.toISOString() },
    })
        .sort({ createdAt: 'desc' })
        .limit(pageSize + 1) // Use 1 more requirement for indication of hasMore
        .exec();
    const hasMore: boolean = articles.length === pageSize + 1;
    const promises: Promise<User | undefined>[] = articles.map(
