...
import ThreadResponse from '../models/Response/ThreadResponse';
...
const threadActionCreator: ThreadActionCreator = {
    getThreads(pageIndex: number, pageSize: number): any {
        return (dispatch: Dispatch<any>): void => {
            ...
            fetch(
                `/api/thread?pageIndex=${pageIndex}&pageSize=${pageSize}`,
                undefined,
                'GET',
                /*withToken*/ true
            )
                .then((json: ThreadResponse) => {
                ...
                .catch((error: Error) => {
                    dispatch(
                        actions.handleFetchError(GET_THREADS_FAILED, error)
                    );
                });
        };
    },
        ...
        return (dispatch: Dispatch<any>): void => {
            ...
            fetch(
                ...
                .catch((error: Error) => {
                    ...
                    return dispatch(
                        actions.handleFetchError(ADD_THREAD_FAILED, error)
                    );
                });
        };
    },
    rateThread(rating: number, id: string, user: string): any {
        return (dispatch: Dispatch<any>): void => {
            fetch(
                `/api/thread/rate?id=${id}&rating=${rating}`,
                undefined,
                'GET',
                /*withToken*/ true
            )
                ...
                .catch((error: Error) => {
                    dispatch(
                        actions.handleFetchError(RATE_THREAD_FAILED, error)
                    );
                });
        };
    },
        ...
        return (dispatch: Dispatch<any>): void => {
            ...
            fetch(
                ...
                .catch((error: Error) => {
                    ...
                    return dispatch(
                        actions.handleFetchError(REMOVE_THREAD_FAILED, error)
                    );
                });
        };
    },
